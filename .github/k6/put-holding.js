...
// med 33.5 ms with Java
// med 25.9 ms with SQL
...
export default function() {
  ...
  const res = http.put('http://localhost:8081/holdings-storage/holdings/' + id, JSON.stringify(holding), params);
  const ok = check(res, { 'expect status 204': (r) => r.status === 204 });
  if (! ok) {
    console.log('status: ' + res.status + ', body: ' + res.body);
  }
}
